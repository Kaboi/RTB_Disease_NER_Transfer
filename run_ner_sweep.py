...
def main():
    ...
    model_args = ModelArguments(
        model_name_or_path=config_data["model_name_or_path"],
        config_name=config_data.get("config_name", None),
        task_type=config_data.get("task_type", "NER"),
        tokenizer_name=config_data.get("tokenizer_name", None),
        use_fast=config_data.get("use_fast", False),
        cache_dir=config_data.get("cache_dir", None),
    )

    data_args = DataTrainingArguments(
        data_dir=config_data["data_dir"],
        labels=config_data.get("labels", None),
        max_seq_length=max_seq_length,  # updated from sweep
        overwrite_cache=config_data.get("overwrite_cache", False),
    )

    training_args = TrainingArguments(
        output_dir=config_data["output_dir"],
        num_train_epochs=num_train_epochs,  # updated from sweep
        per_device_train_batch_size=per_device_train_batch_size,  # updated from sweep
        learning_rate=learning_rate,  # updated from sweep
